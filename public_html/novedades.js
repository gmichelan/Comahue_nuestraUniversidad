function mostrarNovedades(){  
    ...
    $.ajax({
        ...
        success: function(response){ console.log("exitoso");
           $(response.noticias).each(function(index, item){
            ...
            var h4= $(document.createElement('h4')).text(item.titulo); 
            var p=$(document.createElement('p'));
            var a= $(document.createElement('a')).text('Ir a la fuente');
            a.attr("onclick", "navigator.app.loadUrl('"+ item.url+"', {openExternal: true});");
            p.append(a1);
            p.append(item.copete.slice(0,256)+'...');
            p.append(a);
            div.append(h4);
            div.append(p);
